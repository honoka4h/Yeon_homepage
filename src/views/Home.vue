<script setup>
import { onMounted } from 'vue';
import bgms from '@/bgm.js';

onMounted(() => {
  document.title = "연이의 두근두근 ♥ 귀여운 사이트";

  const startBgm = () => {
    for (let key in bgms) {
      if (!bgms[key].paused) bgms[key].pause();
      bgms[key].currentTime = 0;
    }
    
    bgms.audio01.play();
    document.removeEventListener('click', startBgm);
  }

  document.addEventListener('click', startBgm);
});

// document.addEventListener('mousemove', e => {
//   const spark = document.createElement('div');
//   spark.className = 'spark';
//   spark.style.left = `${e.clientX}px`;
//   spark.style.top = `${e.clientY}px`;
  
//   document.body.appendChild(spark);

//   setTimeout(() => spark.remove(), 1000);
// });

function nomade() {
  alert("아직 다 안만들어졌는데용~")
}

</script>

<template>
  <div class="home-wrapper">
    <div class="spark"></div>
    <div class="cute-01"></div>
    <div class="welcometitle">
      <h1>연이의 귀여운 사이트</h1>
      <marquee behavior="scroll" direction="left" scrollamount="5">
        어서오세요 주인님~ ✧ ✧ ✧ 반짝반짝 귀여움 가득한 연이의 사이트에 오신걸 환영해요!
      </marquee>
    </div>

    <div class="image-container">
      <img src="@/assets/images/character_01.png" class="left"></img>
      <img src="@/assets/images/character_02.png" class="right"></img>
    </div>

    <div class="content-row">
      <div class="text-block">
        <p>♥ ♥ 화면 아무곳이나 클릭 시 노래 자동 재생 !! ♥ ♥</p>
        <h3 class="title-text">이곳은 어떤 곳이에요?</h3>
        <p>연이의 사이트에 오신걸 환영해요 !!
          <br>저는 연이라고 하고, 이곳은 저의 개인적인 기록 공간입니다.
          <br>이곳에서는 저의 귀여운 사이트와 캐릭터들, 그리고 블로그를 볼수가 있어요 !
          <br>아직 다 완성이 되진 않았지만, 게임과 상점들도 만들 예정이에요 !
        </p>
        <p>이 사이트는 데스크탑 기준으로 만들어졌습니다.
          <br>컴퓨터나 노트북으로 보시는 것을 추천드립니다 !</p>
      </div>

      <div class="navbar">
        <h3>Navbar</h3>
        <router-link to="/about"><button>About</button></router-link>
        <button @click="nomade">Art</button>
        <button @click="nomade">Blog</button>
        <router-link to="/game"><button>Game</button></router-link>
        <img src="@/assets/images/cute_01.gif"></img>
      </div>
    </div>

    <div class="main-box">
      <marquee behavior="scroll" direction="left" scrollamount="5" loop="infinite">
        <img src="@/assets/images/sticker_01.gif">
        <img src="@/assets/images/sticker_02.jpeg">
        <img src="@/assets/images/sticker_03.jpeg">
        <img src="@/assets/images/sticker_04.png">
        <img src="@/assets/images/sticker_05.gif">
        <img src="@/assets/images/sticker_06.jpeg">
      </marquee>

      <div class="visitviewer">
        <p>당신은 </p>
        <a href="https://www.hitwebcounter.com/" target="_blank">
          <img src="https://hitwebcounter.com/counter/counter.php?page=21442985&style=0006&nbdigits=6&type=page&initCount=0" title="Counters" Alt="Counters" border="0" />
        </a>
        <p> 번째 방문자 입니다! 축하드려요!</p>
      </div>
      <p>아직 사이트를 제작중이라 여러 기기에서 호환성 검사가 되어있지 않습니다 !!
        <br>글자, 사진 등 UI 등 뭉개짐 발생 가능 !!
      </p>
      <h3 class="title-text">↓↓ 이곳에서 방명록 작성가능 !! ↓↓</h3>
      <iframe src="https://honoka4h.atabook.org/" width="300px" height="400px"></iframe>
      <div class="img-box">
        <h3 class="title-text">♥ 좋아하는 사진 모음 ♥</h3>
        <div class="img-box2">
          <img src="@/assets/images/cute_03.gif" width="150px"></img>
          <img src="@/assets/images/cute_04.gif" width="100px"></img>
          <img src="@/assets/images/cute_05.gif" width="165px"></img>
          <img src="@/assets/images/cute_06.gif" width="100px"></img>
          <img src="@/assets/images/cute_07.gif" width="100px"></img>
          <img src="@/assets/images/cute_08.jpg" width="100px"></img>
        </div>
      </div>
    </div>
    <div class="ending-bar">
      <marquee behavior="scroll" direction="left" scrollamount="5">
        노래: 로미오와 신데렐라 (feat.하츠네 미쿠) / 모바일 호환성 확인되지 않음.
      </marquee>
    </div>
  </div>
</template>

<style>
@keyframes swing-scale {
  0%   { transform: scale(1) rotate(0deg); }
  25%  { transform: scale(1.1) rotate(10deg); }
  50%  { transform: scale(1.1) rotate(-10deg); }
  75%  { transform: scale(1.1) rotate(10deg); }
  100% { transform: scale(1) rotate(0deg); }
}

@media (max-width: 1199px) {
  .image-container img {
    display: none;
  }
}

.home-wrapper {
  background-color: pink;
  background-image: url('@/assets/images/wallpaper_01.jpg');
  background-repeat: repeat;
  background-size: auto;
  display: flex;
  flex-direction: column;
  justify-content: flex-start;
  align-items: center;
  width: 100vw;
  min-height: 100vh;
  font-family: 'neodgm', sans-serif;
}

.cute-01 {
  background-image: url('@/assets/images/main_01.png');
  background-repeat: repeat;
  background-size: contain;
  height: 30px;
  width: 100vw;
  left: 0;
  position: absolute;
}

* {
  cursor: url('/src/assets/images/cursor.png') 0 0, auto !important;
}

/* .spark {
  position: absolute;
  width: 15px;
  height: 15px;
  background: hotpink;
  clip-path: polygon(
    50% 0%, 61% 35%, 98% 35%, 68% 57%, 
    79% 91%, 50% 70%, 21% 91%, 32% 57%, 
    2% 35%, 39% 35%
  );
  pointer-events: none;
  opacity: 0.9;
  animation: fall 1s forwards;
}

@keyframes fall {
  0% { transform: translateY(0px) rotate(0deg); opacity: 1; }
  100% { transform: translateY(100px) rotate(360deg); opacity: 0; }
} */

.welcometitle {
  margin-top: 50px;
  display: flex;
  flex-direction: column;
  justify-content: center;
}

.title-text {
  color: pink;
  text-shadow: -3px 0px white;
  font-size: 1.5rem;
}

.img-box {
  border: 1px solid hotpink;
  display: flex;
  justify-content: flex-start;
  flex-direction: column;
  align-items: center;
  background-image: url('@/assets/images/cute_02.png');
  width: 300px;
  height: 400px;
  float: right;
  margin-right: 50px;
}

.img-box2 {
  width: 100%;
  display: flex;
  justify-content: center;
  align-items: flex-start;
  gap: 5px;
  flex-wrap: wrap;
}

.navbar {
  border: 1px solid hotpink;
  display: flex;
  justify-content: center;
  flex-direction: column;
  align-items: center;
  background-image: url('@/assets/images/cute_02.png');
  width: 150px;
  height: 250px;
  margin-top: 50px;
}

.navbar button {
  width: 100px;
  height: 30px;
  font-family: 'neodgm', sans-serif;
  margin-top: 5px;
  margin-bottom: 5px;
  color: hotpink;
  background-color: rgb(255, 218, 224);
  border: 1px solid hotpink;
}

.main-box {
  margin-top: 20px;
}

.content-row {
  display: flex;
  justify-content: center;
  align-items: flex-start;
  width: 100%;
}

.visitviewer {
  display: flex;
  align-items: center

}

.visitviewer p {
  font-size: 1.5rem;
  color:aqua;
  text-shadow: -3px 0px white;
}

.image-container {
  display: flex;
  gap: 5px;
  left: 0;
  width: 100%;
  justify-content: space-between;
  margin-top: 30px;
  width: 100vw;
  z-index: 0;
}
.image-container img {
  width: 200px;
  height: auto;
}

.image-container img.left {
  position: absolute;
  left: 20px;
}

.image-container img.right {
  position: absolute;
  right: 20px;
  margin-top: 150px;
}

.image-container img:hover {
  animation: swing-scale 2s ease-in-out infinite;
  transform-origin: center;
}

.ending-bar {
  bottom: 50px;
  display: flex;
  justify-content: flex-end;
  align-items: flex-end;
  margin-top: 20px;
  margin-bottom: 30px;
}


</style>